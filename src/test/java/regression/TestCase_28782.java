...
public class TestCase_28782 extends TestUtils
{
	@Test	
	@Parameters("browser")

	public void Cockpit_DataView_Olap_Functions_ClientRules_DimLevel(String browser) throws Exception
	{
	    test =extent.createTest("TestCase_28782: Cockpit DataView Olap Functions for ClientRules_DimLevel");
		Login log = new Login();
		...
		log.createCubeReport("//div[text()='A - Training Sales']","ClientRules_DimLevel_check",true);
       ...
       try{
			for(int i=0;i<=SelectableElementvalue.size()-1;i++)
			{
				...
				if(valueofcontainerele.contains("DimLevel"))
				{
					System.out.println("DimLevel is present");
					test.info("DimLevel is present");
					
					waits.waitUntil_Elmentis_visibilityOfElement(10,"//span[text()='DimLevel']");
					log.driver.findElement(By.xpath("//span[text()='DimLevel']")).click();
					Thread.sleep(1000);
		...
		log.driver.findElement(By.xpath("//span[text()='Name:']/ancestor::div[3]/div[2]//input[1]")).sendKeys("DimLevel");
		Thread.sleep(500);
		...
		action.click();
		//string s="DimLevel( "A - Training Sales.[Time].[Time]", "Q1 2013")"
		action.sendKeys("DimLevel( \"A - Training Sales.[Time].[Time]\",\"Q1 2013\" )");
		...
		for (int i =0;i<=selectble_ele.size()-1;i++) 
		{
			...
			selectable.add(aa);
			if(selectable.contains("DimLevel"))
			{
				assertTrue(true);
				System.out.println("DimLevel is available");
				
			}
			
		}
		Thread.sleep(2000);
		waits.waitUntil_Elmentis_visibilityOfElement(10,"//div[text()='DimLevel']/../div[1]");
		log.driver.findElement(By.xpath("//div[text()='DimLevel']/../div[1]")).click();
		Thread.sleep(1000);
		waits.waitUntil_Elmentis_visibilityOfElement(10,"//div[text()='DimLevel']/../div[1]");
		assertTrue(log.driver.findElement(By.xpath("//div[text()='DimLevel']/../div[1]/input[1]")).isSelected());
		System.out.println("DimLevel Checkbox is Selected in the list of selectable elements");
		...
		{
			if(webElement.getText().equals("Purchase price")||webElement.getText().equals("DimLevel"))
			{
				assertTrue(true);
			}
		}
		...
		System.out.println("column with the name DimLevel");
		test.info("column with the name DimLevel");
		String text=log.driver.findElement(By.xpath("(//div[text()='2014']/ancestor::div[4]/div/div[4]/div/div/div[2])[1]")).getText();
		...
		waits.waitUntil_Elmentis_visibilityOfElement(10,"(//div[text()='DimLevel'])[1]");
		log.driver.findElement(By.xpath("(//div[text()='DimLevel'])[1]")).click();
		Thread.sleep(1000);
		...
		log.driver.findElement(By.xpath("//span[text()='Alias:']/following::div[4]/input")).sendKeys("Q1 2013 is at Level");
		...
		{
			if(webElement.getText().equals("Purchase price")||webElement.getText().equals("Q1 2013 is at Level"))
			{
				assertTrue(true);
			}
		}
		...
		System.out.println("column with the name Units sold and Q1 2013 is at Level");
		test.info("column with the name Units sold and Q1 2013 is at Level");
		
		
		String text1=log.driver.findElement(By.xpath("(//div[text()='2014']/ancestor::div[4]/div/div[4]/div/div/div[2])[1]")).getText();
		assertTrue(text1.contains("2"));
