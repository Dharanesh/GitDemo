...
public class TestCase_28774 extends TestUtils
{
	@Test	
	@Parameters("browser")

	public void Cockpit_DataView_Olap_Functions_ClientRules_DimWeight(String browser) throws Exception
	{
	    test =extent.createTest("TestCase_28774: Cockpit DataView Olap Functions for ClientRules_DimWeight");
		Login log = new Login();
		...
		log.createCubeReport("//div[text()='A - Training Sales']","ClientRules_DimWeight_check",false);
       ...
       try{
			for(int i=0;i<=SelectableElementvalue.size()-1;i++)
			{
				...
				if(valueofcontainerele.contains("DimWeight"))
				{
					System.out.println("DimWeight is present");
					test.info("DimWeight is present");
					
					waits.waitUntil_Elmentis_visibilityOfElement(10,"//span[text()='DimWeight']");
					log.driver.findElement(By.xpath("//span[text()='DimWeight']")).click();
					Thread.sleep(1000);
		...
		log.driver.findElement(By.xpath("//span[text()='Name:']/ancestor::div[3]/div[2]//input[1]")).sendKeys("DimWeight");
		Thread.sleep(500);
		...
		action.click();
		//string s="DimWeight( "A - Training Sales.[Time].[Time]", "Q1 2014", "2014")"
		action.sendKeys("DimWeight( \"A - Training Sales.[Time].[Time]\",\"Q1 2014\",\"2014\")");
		...
		for (int i =0;i<=selectble_ele.size()-1;i++) 
		{
			...
			selectable.add(aa);
			if(selectable.contains("DimWeight"))
			{
				assertTrue(true);
				System.out.println("DimWeight is available");
				
			}
			
		}
		Thread.sleep(2000);
		waits.waitUntil_Elmentis_visibilityOfElement(10,"//div[text()='DimWeight']/../div[1]");
		log.driver.findElement(By.xpath("//div[text()='DimWeight']/../div[1]")).click();
		Thread.sleep(1000);
		waits.waitUntil_Elmentis_visibilityOfElement(10,"//div[text()='DimWeight']/../div[1]");
		assertTrue(log.driver.findElement(By.xpath("//div[text()='DimWeight']/../div[1]/input[1]")).isSelected());
		System.out.println("DimWeight Checkbox is Selected in the list of selectable elements");
		...
		waits.waitUntil_Elmentis_visibilityOfElement(10,"(//div[text()='DimWeight'])[2]");
		log.driver.findElement(By.xpath("(//div[text()='DimWeight'])[2]")).click();
		Thread.sleep(1000);
		...
		log.driver.findElement(By.xpath("//span[text()='Alias:']/following::div[4]/input")).sendKeys("The element has a weighting of 'XX' to its parent element");
		...
		{
			if(webElement.getText().equals("Purchase price")||webElement.getText().equals("The element has a weighting of 'XX' to its parent element"))
			{
				assertTrue(true);
			}
		}
		...
		System.out.println("column with the name The element has a weighting of 'XX' to its parent element");
		test.info("column with the name The element has a weighting of 'XX' to its parent element");
		
		String text=log.driver.findElement(By.xpath("//div[text()='2014']/ancestor::div[4]/div/div[4]/div/div/div[2]")).getText();
		assertEquals(text,"1.00");
