...
public class TestCase_28775  extends TestUtils
{
	@Test	
	@Parameters("browser")

	public void Cockpit_DataView_Olap_Functions_ClientRules_MaxChild(String browser) throws Exception
	{
	    test =extent.createTest("TestCase_28775: Cockpit DataView Olap Functions for ClientRules_MaxChild");
		Login log = new Login();
		...
		log.createCubeReport("//div[text()='A - Training Sales']","ClientRules_MaxChild_check",true);
		...
		waits.waitUntil_Elmentis_visibilityOfElement(10,"//div[text()='Audi']");
		log.driver.findElement(By.xpath("//div[text()='Audi']")).click();
		Thread.sleep(1000);
       ...
       try{
			for(int i=0;i<=SelectableElementvalue.size()-1;i++)
			{
				...
				if(valueofcontainerele.contains("MaxChild"))
				{
					System.out.println("MaxChild is present");
					test.info("MaxChild is present");
					
					waits.waitUntil_Elmentis_visibilityOfElement(10,"//span[text()='MaxChild']");
					log.driver.findElement(By.xpath("//span[text()='MaxChild']")).click();
					Thread.sleep(1000);
		...
		log.driver.findElement(By.xpath("//span[text()='Name:']/ancestor::div[3]/div[2]//input[1]")).sendKeys("MaxChild");
		Thread.sleep(500);
		...
		action.click();
		//string s="MaxChild( "A - Training Sales.[Products].[Product]", "Audi" )"
		action.sendKeys("MaxChild( \"A - Training Sales.[Products].[Product]\",\"Audi\" )");
		...
		for (int i =0;i<=selectble_ele.size()-1;i++) 
		{
			...
			selectable.add(aa);
			if(selectable.contains("MaxChild"))
			{
				assertTrue(true);
				System.out.println("MaxChild is available");
				
			}
			
		}
		Thread.sleep(2000);
		waits.waitUntil_Elmentis_visibilityOfElement(10,"//div[text()='MaxChild']/../div[1]");
		log.driver.findElement(By.xpath("//div[text()='MaxChild']/../div[1]")).click();
		Thread.sleep(1000);
		waits.waitUntil_Elmentis_visibilityOfElement(10,"//div[text()='MaxChild']/../div[1]");
		assertTrue(log.driver.findElement(By.xpath("//div[text()='MaxChild']/../div[1]/input[1]")).isSelected());
		System.out.println("MaxChild Checkbox is Selected in the list of selectable elements");
		...
		waits.waitUntil_Elmentis_visibilityOfElement(10,"(//div[text()='MaxChild'])[2]");
		log.driver.findElement(By.xpath("(//div[text()='MaxChild'])[2]")).click();
		Thread.sleep(1000);
		...
		log.driver.findElement(By.xpath("//span[text()='Alias:']/following::div[4]/input")).sendKeys("Maximum Child Value");
		...
		waits.waitUntil_Elmentis_visibilityOfElement(10,"(//div[text()='Audi'])[1]/ancestor::div[3]/div[3]/div/div");
		List<WebElement> TableColumnHdr = log.driver.findElements(By.xpath("(//div[text()='Audi'])[1]/ancestor::div[3]/div[3]/div/div"));
		for (WebElement webElement : TableColumnHdr)
		{
			if(webElement.getText().equals("Purchase price")||webElement.getText().equals("Maximum Child Value"))
			{
				assertTrue(true);
			}
		}
		...
		System.out.println("column with the name Maximum Child Value");
		test.info("column with the name Maximum Child Value");
		
		String text=log.driver.findElement(By.xpath("//div[text()='Audi']/ancestor::div[4]/div/div[4]/div/div/div[2]")).getText();
		assertEquals(text,"4,250,420.85");
		...
		/*List<WebElement> units =log. driver.findElements(By.xpath("//div[@class='dojoSplitPane']/div[3]/div[2]/div/div/div/div[4]/div/div"));
		ArrayList<Integer> value = new ArrayList<>();
		int count = 0;
		for(int i=1;i<units.size()-1;i++)
		{
		  String a =	units.get(i).getText();
		...
		System.out.println("The list of Value "+integer);
				
		}
		
		String checkvalue = units.get(units.size()-1).getText();
		Integer verify = Integer.parseInt(checkvalue);
		
		int i = verify;
		int maxvalue = Collections.max(value);
	
		assertEquals(i,maxvalue);
