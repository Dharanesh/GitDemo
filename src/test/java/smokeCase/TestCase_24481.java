...
public class TestCase_24481 extends TestUtils
{
	@Test
	@Parameters("browser")
	public void Testcase_verify_creating_Title_Layout_component(String browser) throws Exception
	{
		...
		test =extent.createTest("TestCase_24481: Web_Testcase to verify PDF Export for functionality of <Analytical Visualization Gradient_New>");
		Login log = new Login();
...
//		Open Gradient report
		...
		clickableClick(By.xpath("//span[text()='Eigene Berichte']"));
		Thread.sleep(500);
		
		log.driver.findElement(By.xpath("(//span[@class='k-icon k-i-expand'])[1]")).click();
		Thread.sleep(2000);
//		ui.rightClick("//span[text()='Gradient_Report']");
...
//		te.add("Export report definition(s) without data into a special file (CWDX file).");
		Thread.sleep(3000);
		int j =1;
		...
		assertEquals(log.driver.findElement(By.xpath("(//div[@class='w20_webedit']/input)[1]")).getAttribute("value"), "Gradient_Report");
		...
		for(int i =0;i<=buttonlistsize11-1;i++) 
		{
		 ...
		 log.driver.findElement(By.xpath("//label[text()='No iteration']/../input")).click();
		 Thread.sleep(2000);
		 ...
		 assertTrue(log.driver.findElement(By.xpath("(//*[text()='Export even if no data']/following::div[@class='w20_checkbox']/label)[1]")).isEnabled());
			...
			Thread.sleep(5000);
			assertEquals(log.driver.findElement(By.xpath("//tbody[@role='rowgroup']/tr[1]/td[3]")).getText(), "Executing export 'Gradient_Report'");
			assertEquals(log.driver.findElement(By.xpath("//tbody[@role='rowgroup']/tr[2]/td[3]")).getText(), "Export report 'Gradient_Report'");
