...
public class TestCase_24490 extends TestUtils {
	...
	public void Web_Testcase_verify_functionality_Analytical_Visualization_Gradient_Edit(String browser) throws Exception
	{
		...
		test =extent.createTest("TestCase_24490_Web_Testcase to verify PDF Export for functionality of <Analytical Visualization Threshold_New'>");
		Login log = new Login();
...
//		Open Gradient report
		...
		log.driver.findElement(By.xpath("//span[text()='Threshold_New']")).click();
...
//			System.out.println(all.get(i).getText());
		...
		assertEquals(log.driver.findElement(By.xpath("(//div[@class='w20_webedit']/input)[1]")).getAttribute("value"), "Threshold_New");
		...
		for(int i =0;i<=buttonlistsize11-1;i++) 
		{
		 ...
		 assertTrue(log.driver.findElement(By.xpath("(//*[text()='Export even if no data']/following::div[@class='w20_checkbox']/label)[1]")).isEnabled());
			...
			Thread.sleep(5000);
			assertEquals(log.driver.findElement(By.xpath("//tbody[@role='rowgroup']/tr[1]/td[3]")).getText(), "Executing export 'Threshold_New'");
			assertEquals(log.driver.findElement(By.xpath("//tbody[@role='rowgroup']/tr[2]/td[3]")).getText(), "Export report 'Threshold_New'");
