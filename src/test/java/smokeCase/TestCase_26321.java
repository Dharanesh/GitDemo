...
public class TestCase_26321 extends TestUtils
{
	@Test
	@Parameters("browser")
	public void Testcase_verify_creating_Title_Layout_component(String browser) throws Exception
	{
		...
		test =extent.createTest("26321: Web_Testcase to verify PDF Export for functionality of <Analytical Visualization InCell Bar>");
		Login log = new Login();
...
//		Open Gradient report
		...
		log.driver.findElement(By.xpath("//span[text()='InCell_Bar_New']")).click();
...
//			System.out.println(all.get(i).getText());
		...
		assertEquals(log.driver.findElement(By.xpath("(//div[@class='w20_webedit']/input)[1]")).getAttribute("value"), "InCell_Bar_New");
		...
		for(int i =0;i<=buttonlistsize11-1;i++) 
		{
		 ...
		 assertTrue(log.driver.findElement(By.xpath("(//*[text()='Export even if no data']/following::div[@class='w20_checkbox']/label)[1]")).isEnabled());
			...
			Thread.sleep(5000);
			assertEquals(log.driver.findElement(By.xpath("//tbody[@role='rowgroup']/tr[1]/td[3]")).getText(), "Executing export 'InCell_Bar_New'");
			assertEquals(log.driver.findElement(By.xpath("//tbody[@role='rowgroup']/tr[2]/td[3]")).getText(), "Export report 'InCell_Bar_New'");
